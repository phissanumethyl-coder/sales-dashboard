...
const api = {
  ...
  getBranches: async (token) => {
    try {
      ...
      return Array.isArray(await res.clone().json()) ? res.json() : [];
    } catch { return []; }
  },
...
const formatCurrency = (num) => new Intl.NumberFormat('th-TH').format(Math.round(num || 0));
const getPerformanceColor = (pct) => parseFloat(pct) >= 100 ? '#10b981' : parseFloat(pct) >= 80 ? '#f59e0b' : '#ef4444';
const getExpenseColor = (pct) => parseFloat(pct) <= 15 ? '#10b981' : parseFloat(pct) <= 25 ? '#f59e0b' : '#ef4444';
const getStatus = (pct) => {
  const p = parseFloat(pct);
  return p >= 100 ? { text: 'ถึงเป้า', emoji: '🎯', cls: 'success' } : p >= 80 ? { text: 'ใกล้เป้า', emoji: '📈', cls: 'warning' } : { text: 'ต่ำกว่าเป้า', emoji: '⚠️', cls: 'danger' };
};
...
const DataEntryPage = ({ token, period, onDataChange }) => {
  ...
  const handleAddBranch = async (e) => {
    e.preventDefault();
    setLoading(true);
    const result = await api.addBranch(token, branchForm);
    if (!result.error) {
      showMessage('✅ เพิ่มสาขาสำเร็จ!');
      setBranchForm({ name: '', color: '#3b82f6' });
  ...
  const handleUpdateBranch = async (e) => {
    e.preventDefault();
    setLoading(true);
    await api.updateBranch(token, editingBranch.id, editingBranch);
  ...
  const handleAddEmployee = async (e) => {
    e.preventDefault();
    setLoading(true);
    const result = await api.addEmployee(token, {
      branch_id: parseInt(employeeForm.branch_id),
      name: employeeForm.name,
      target_facebook: parseFloat(employeeForm.target_facebook) || 0,
      target_shopee: parseFloat(employeeForm.target_shopee) || 0,
      target_lazada: parseFloat(employeeForm.target_lazada) || 0,
    });
    if (!result.error) {
      showMessage('✅ เพิ่มพนักงานสำเร็จ!');
      setEmployeeForm({ branch_id: '', name: '', target_facebook: '', target_shopee: '', target_lazada: '' });
      loadData(); onDataChange();
    } else { showMessage('❌ ' + result.error, true); }
    setLoading(false);
  };
  ...
  const handleUpdateEmployee = async (e) => {
    e.preventDefault();
    setLoading(true);
    await api.updateEmployee(token, editingEmployee.id, {
      name: editingEmployee.name,
      target_facebook: parseFloat(editingEmployee.target_facebook) || 0,
      target_shopee: parseFloat(editingEmployee.target_shopee) || 0,
      target_lazada: parseFloat(editingEmployee.target_lazada) || 0,
    });
    showMessage('✅ แก้ไขเป้าหมายสำเร็จ!');
  ...
  const handleAddSales = async (e) => {
    e.preventDefault();
    setLoading(true);
    for (const [ch, amt] of [['facebook', salesForm.facebook], ['shopee', salesForm.shopee], ['lazada', salesForm.lazada]]) {
      if (amt && parseFloat(amt) >= 0) {
        await api.addSale(token, { employee_id: parseInt(salesForm.employee_id), channel: ch, amount: parseFloat(amt), year: period.year, month: period.month });
      }
    }
  ...
  const handleAddExpenses = async (e) => {
    e.preventDefault();
    setLoading(true);
    for (const [t, amt] of [['cost', expenseForm.cost], ['ads', expenseForm.ads], ['fees', expenseForm.fees]]) {
      if (amt && parseFloat(amt) >= 0) {
        await api.addExpense(token, { employee_id: parseInt(expenseForm.employee_id), type: t, amount: parseFloat(amt), year: period.year, month: period.month });
      }
    }
    ...
    <div className="data-entry">
      ...
      {activeForm === 'sales' && (
        <form className="entry-form" onSubmit={handleAddSales}>
          <h3>💰 บันทึกยอดขาย - {getMonthLabel()} {period.year}</h3>
      ...
      {activeForm === 'expenses' && (
        <form className="entry-form" onSubmit={handleAddExpenses}>
          <h3>📉 บันทึกค่าใช้จ่าย - {getMonthLabel()} {period.year}</h3>
          {employees.length === 0 ? <div className="empty-state"><p>⚠️ กรุณาเพิ่มสาขาและพนักงานก่อน</p></div> : (
            <>
              <div className="form-group">
                <label>👤 พนักงาน</label>
      ...
      {/* EMPLOYEE - ตั้งเป้าหมายรายคน */}
      {activeForm === 'employee' && (
        <div className="entry-form">
          <h3>👤 จัดการพนักงาน & เป้าหมาย</h3>
          
          {editingEmployee ? (
            <form onSubmit={handleUpdateEmployee} className="edit-modal">
              <h4>✏️ แก้ไขเป้าหมาย: {editingEmployee.name}</h4>
              <div className="form-group">
                <label>👤 ชื่อ</label>
                <input type="text" value={editingEmployee.name} onChange={e => setEditingEmployee({...editingEmployee, name: e.target.value})} required />
              </div>
              <div className="form-row">
                <div className="form-group"><label>📘 เป้า Facebook</label><input type="number" min="0" value={editingEmployee.target_facebook} onChange={e => setEditingEmployee({...editingEmployee, target_facebook: e.target.value})} /></div>
                <div className="form-group"><label>🛒 เป้า Shopee</label><input type="number" min="0" value={editingEmployee.target_shopee} onChange={e => setEditingEmployee({...editingEmployee, target_shopee: e.target.value})} /></div>
                <div className="form-group"><label>🛍️ เป้า Lazada</label><input type="number" min="0" value={editingEmployee.target_lazada} onChange={e => setEditingEmployee({...editingEmployee, target_lazada: e.target.value})} /></div>
              </div>
              <div className="target-sum">เป้ารวม: ฿{formatCurrency((parseFloat(editingEmployee.target_facebook)||0)+(parseFloat(editingEmployee.target_shopee)||0)+(parseFloat(editingEmployee.target_lazada)||0))}</div>
              <div className="btn-row">
                <button type="submit" className="submit-btn" disabled={loading}>{loading ? '⏳...' : '💾 บันทึก'}</button>
                <button type="button" className="cancel-btn" onClick={() => setEditingEmployee(null)}>❌ ยกเลิก</button>
              </div>
            </form>
          ) : branches.length === 0 ? (
            <div className="empty-state"><p>⚠️ กรุณาเพิ่มสาขาก่อน</p></div>
          ) : (
            <form onSubmit={handleAddEmployee}>
              ...
              <div className="form-row">
                <div className="form-group"><label>📘 เป้า Facebook</label><input type="number" min="0" value={employeeForm.target_facebook} onChange={e => setEmployeeForm({...employeeForm, target_facebook: e.target.value})} placeholder="0" /></div>
                <div className="form-group"><label>🛒 เป้า Shopee</label><input type="number" min="0" value={employeeForm.target_shopee} onChange={e => setEmployeeForm({...employeeForm, target_shopee: e.target.value})} placeholder="0" /></div>
                <div className="form-group"><label>🛍️ เป้า Lazada</label><input type="number" min="0" value={employeeForm.target_lazada} onChange={e => setEmployeeForm({...employeeForm, target_lazada: e.target.value})} placeholder="0" /></div>
              </div>
              <button type="submit" className="submit-btn" disabled={loading}>{loading ? '⏳...' : '➕ เพิ่มพนักงาน'}</button>
            </form>
          )}
          ...
          {employees.length > 0 && !editingEmployee && (
            <div className="data-list">
              ...
              {employees.map(emp => (
                <div key={emp.id} className="data-item emp-item">
                  <div className="emp-info">
                    <strong>{emp.name}</strong> <small>({emp.branch_name})</small>
                    <div className="emp-targets">
                      <span style={{color:'#1877f2'}}>FB: ฿{formatCurrency(emp.target_facebook)}</span>
                      <span style={{color:'#ee4d2d'}}>Shopee: ฿{formatCurrency(emp.target_shopee)}</span>
                      <span style={{color:'#0f146d'}}>Lazada: ฿{formatCurrency(emp.target_lazada)}</span>
                      <strong>รวม: ฿{formatCurrency((emp.target_facebook||0)+(emp.target_shopee||0)+(emp.target_lazada||0))}</strong>
                    </div>
                  </div>
                  <div className="item-actions">
                    <button className="edit-btn" onClick={() => setEditingEmployee({...emp})}>✏️</button>
      ...
      {/* BRANCH - ไม่มีเป้าหมายแล้ว */}
      {activeForm === 'branch' && (
        <div className="entry-form">
          <h3>🏢 จัดการสาขา</h3>
          <p style={{color:'#64748b',fontSize:'12px',marginBottom:'16px'}}>💡 เป้าหมายสาขา = รวมเป้าหมายพนักงานทุกคนในสาขา</p>
          
          {editingBranch ? (
            <form onSubmit={handleUpdateBranch} className="edit-modal">
              <h4>✏️ แก้ไขสาขา</h4>
              <div className="form-group">
            ...
            <form onSubmit={handleAddBranch}>
              ...
              <button type="submit" className="submit-btn" disabled={loading}>{loading ? '⏳...' : '➕ เพิ่มสาขา'}</button>
            </form>
          )}
          ...
          {branches.length > 0 && !editingBranch && (
            <div className="data-list">
              ...
              {branches.map(b => {
                ...
                const totalTarget = branchEmps.reduce((sum, e) => sum + (e.target_facebook||0) + (e.target_shopee||0) + (e.target_lazada||0), 0);
                return (
                  <div key={b.id} className="data-item">
                    <div style={{display:'flex',alignItems:'center',gap:'8px',flex:1}}>
                      ...
                      <small>({branchEmps.length} คน | เป้ารวม: ฿{formatCurrency(totalTarget)})</small>
                    </div>
                    <div className="item-actions">
...
export default function App() {
  ...
  const totalExpenseBreakdown = {
    cost: branches.reduce((s, b) => s + (b.expenses?.cost || 0), 0),
    ads: branches.reduce((s, b) => s + (b.expenses?.ads || 0), 0),
    fees: branches.reduce((s, b) => s + (b.expenses?.fees || 0), 0),
  };
    ...
    <><style>{styles}</style>
      <div className="dashboard">
        ...
        <div className="filters">
          <div className="filter"><span>📅 ปี:</span><select value={period.year} onChange={e => setPeriod({...period, year: parseInt(e.target.value)})}>{yearOptions.map(y => <option key={y} value={y}>{y}</option>)}</select></div>
          <div className="filter"><span>📆 เดือน:</span><select value={period.month} onChange={e => setPeriod({...period, month: parseInt(e.target.value)})}>{monthNames.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select></div>
            ...
            <>
              ...
              {branches.length > 0 && (
                <div className="chart-section">
                  <h3>📊 เปรียบเทียบยอดขาย vs เป้าหมาย</h3>
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={branches.map(b=>({name:b.name?.replace('สาขา','')||'N/A',ยอดขาย:b.totalSales||0,เป้าหมาย:b.totalTarget||0}))}>
                      ...
                      <Legend/>
                      <Bar dataKey="เป้าหมาย" fill="#64748b" radius={[4,4,0,0]}/>
          ...
          branches.length === 0 ? <div className="empty-dashboard"><div className="empty-icon">🏢</div><h2>ยังไม่มีสาขา</h2><button className="start-btn" onClick={() => setActiveTab('entry')}>➕ เพิ่มสาขา</button></div> : (
            <>
              <div className="branch-grid">
                {branches.map(branch => {
                  const pct = parseFloat(branch.performancePct || 0);
                  const status = getStatus(pct);
                  const diff = branch.diffFromTarget || 0;
                  return (
                    <div key={branch.id} className={`branch-card ${selectedBranch?.id === branch.id ? 'selected' : ''}`} onClick={() => setSelectedBranch(selectedBranch?.id === branch.id ? null : branch)}>
                      ...
                      <div className="progress"><div className="progress-header"><span>เป้า: ฿{formatCurrency(branch.totalTarget)}</span><span style={{color:getPerformanceColor(pct)}}>{pct}%</span></div><div className="progress-bar"><div className="progress-fill" style={{width:`${Math.min(pct,100)}%`,background:getPerformanceColor(pct)}}></div></div><div className="diff-label" style={{color:diff>=0?'#10b981':'#ef4444'}}>{diff>=0?'▲ เกินเป้า':'▼ ขาดเป้า'} ฿{formatCurrency(Math.abs(diff))}</div></div>
                      <div className="branch-expenses-row"><span>📦 {branch.costPct||0}%</span><span>📢 {branch.adsPct||0}%</span><span>💳 {branch.feesPct||0}%</span></div>
                      <div className="branch-stats"><div className="stat"><div className="stat-label">ยอดขาย</div><div className="stat-value">฿{formatCurrency(branch.totalSales)}</div></div><div className="stat"><div className="stat-label">ค่าใช้จ่าย</div><div className="stat-value neg">{branch.totalExpPct||0}%</div></div><div className="stat"><div className="stat-label">กำไร</div><div className="stat-value pos">฿{formatCurrency(branch.netProfit)}</div></div></div>
                    </div>
                  );
                })}
              </div>
              ...
              {selectedBranch && (
                <div className="emp-section">
                  <div className="emp-header"><h3>👥 พนักงาน {selectedBranch.name}</h3><button className="close-btn" onClick={()=>setSelectedBranch(null)}>✕</button></div>
                  {selectedBranch.employees?.length > 0 ? (
                    <div className="emp-grid">
                      {[...selectedBranch.employees].sort((a,b)=>parseFloat(b.performancePct||0)-parseFloat(a.performancePct||0)).map((emp,idx) => {
                        const pct = parseFloat(emp.performancePct || 0);
                        const diff = emp.diffFromTarget || 0;
                        return (
                          <div key={emp.id} className="emp-card">
                            <div className="emp-card-header"><div className="emp-avatar">{emp.name?.charAt(0)||'?'}</div><div><div className="emp-name">{emp.name}</div><div className="emp-rank">อันดับ {idx+1}</div></div><span className={`rank-badge ${idx<3?`rank-${idx+1}`:'rank-n'}`}>{idx+1}</span></div>
                            <div className="emp-performance"><div className="perf-bar"><div className="perf-fill" style={{width:`${Math.min(pct,100)}%`,background:getPerformanceColor(pct)}}></div></div><div className="perf-info"><span style={{color:getPerformanceColor(pct)}}>{pct}%</span><span style={{color:diff>=0?'#10b981':'#ef4444'}}>{diff>=0?'▲':'▼'} ฿{formatCurrency(Math.abs(diff))}</span></div></div>
                            <div className="emp-expenses-row"><span style={{color:'#ef4444'}}>📦 {emp.costPct||0}%</span><span style={{color:'#f59e0b'}}>📢 {emp.adsPct||0}%</span><span style={{color:'#8b5cf6'}}>💳 {emp.feesPct||0}%</span></div>
                            <div className="emp-summary"><div><span>ยอดขาย</span><strong>฿{formatCurrency(emp.totalSales)}</strong></div><div><span>เป้าหมาย</span><strong>฿{formatCurrency(emp.targets?.total)}</strong></div><div><span>กำไร</span><strong style={{color:'#34d399'}}>฿{formatCurrency(emp.netProfit)}</strong></div></div>
                          </div>
                        );
                      })}
                    </div>
                  ) : <p style={{textAlign:'center',color:'#64748b',padding:'20px'}}>ยังไม่มีพนักงาน</p>}
                </div>
              )}
            </>
          )
        ...
        {/* DATA ENTRY */}
        {activeTab === 'entry' && <DataEntryPage token={token} period={period} onDataChange={loadDashboardData} />}
      </div>
    </>
  );
}
...
.emp-item{flex-direction:column;align-items:stretch}.emp-info{margin-bottom:8px}.emp-targets{display:flex;gap:12px;font-size:11px;flex-wrap:wrap;margin-top:4px}
